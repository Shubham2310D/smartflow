...
- **Honest cluster footprints — and the convex hull used where it's right.** On the Hotspot Map, chronic clusters render as fixed-radius circles around the centroid, *not* convex hulls — a hull over road-aligned historical incidents produces giant triangles that overstate the affected area. But on **Live Ops**, there's an optional **per-cluster convex hull** overlay (`scipy.spatial.ConvexHull`) that draws the *current* operational footprint of each active hotspot — the area to cordon and manage as one zone. Two different jobs: a stable chronic footprint (circle) vs. a live cordon boundary (hull computed per cluster, never one city-wide hull that would overstate it).
