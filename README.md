...
- **Honest cluster footprints — and the convex hull used where it's right.** On the Hotspot Map, chronic clusters render as fixed-radius circles around the centroid, *not* convex hulls — a hull over road-aligned historical incidents produces giant triangles that overstate the affected area. Where a hull *is* the right tool is the **diversion reroute** (Live Ops → Compute reroute): there, a **`scipy.spatial.ConvexHull`** over the detour geometry + closure draws the **"diversion-affected area"** — the bounded region the reroute spans, which an operator can cordon/advise around. Right tool, right place: a centroid circle for a chronic footprint, a convex hull for the concrete area a single diversion impacts.
